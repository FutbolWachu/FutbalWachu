...
public class AdministradorDeSaques {
            ...
            if (arena.getEntityInCancha() == null) {
                ...
                switch (zona) {
                    case HORIZONTAL:
                        ...
                        break;
                    case VERTICAL:
                        if(equipo == determinarEquipodeZonaMasCercana(entity)){

                            mecanicas.teleportarJugadorAleatorioNoPortero(equipo, encontrarUbicacionParaSaqueDeBanda(entity));
                        }else{

                            mecanicas.teleportarPortero(equipo, encontrarUbicacionParaSaqueDeMeta(entity));
                        }
    ...
    public Location encontrarUbicacionParaSaqueDeMeta(Entity balon) {
        Location ubicacionBalon = balon.getLocation();
        Location ubicacionMasCercana = null;
        double distanciaMinima = Double.MAX_VALUE; // Inicializamos con el máximo valor posible para comparar.

        for (Region zona : arena.getZones().values()) {
            Location centroZona = zona.getCenter();
            double distanciaActual = centroZona.distance(ubicacionBalon);

            if (distanciaActual < distanciaMinima) {
                distanciaMinima = distanciaActual;
                ubicacionMasCercana = centroZona;
            }
        }

        // Verificar si encontramos alguna ubicación más cercana.
    ...
    public Team determinarEquipodeZonaMasCercana(Entity balon) {
        Location ubicacionBalon = balon.getLocation();

        double distanciaMinimaEsquina = Double.MAX_VALUE;
        Team zonaMasCercana = null;
        Region zonaMasCercanaRegion = null;
        ...
        for (Region zona : arena.getZones().values()) {
            for (Location esquina : zona.getEsquinas()) {
                ...
                if (distancia < distanciaMinimaEsquina) {
                    ...
                    zonaMasCercanaRegion = zona;
                }
            }
        }

        if (zonaMasCercanaRegion != null) {
            for (Map.Entry<Team, Region> entry : arena.getZones().entrySet()) {
                if (entry.getValue().equals(zonaMasCercanaRegion)) {
